...
#define PWM_RES           8
#define DUTY_WEAK         120     // LEVEL3 / 无数据: 最弱
#define DUTY_MED          150    // LEVEL1/2: 中等
#define DUTY_STRONG       255    // LEVEL0: 最强
#define DUTY_DEFAULT      DUTY_WEAK  // 默认振动（无数据时）最弱

// ================= 全局变量 =================
BLEClient* pClient = nullptr;
BLERemoteCharacteristic* pRemoteChar = nullptr;
bool connected = false;
uint8_t currentLevel = 0;  // 默认使用LEVEL0（最强）
...
// LEVEL2 → 中等 (DUTY_MED)
// LEVEL1 → 中等 (DUTY_MED)
// LEVEL0 → 最强 (DUTY_STRONG)
// 无数据时 → 最弱 (DUTY_WEAK)
void setVibration(uint8_t level) {
  uint32_t duty;
  switch (level) {
    case 0: duty = DUTY_STRONG; break;  // LEVEL0: 最强
    case 1: duty = DUTY_MED; break;     // LEVEL1: 中等
    case 2: duty = DUTY_MED; break;     // LEVEL2: 中等
    case 3: duty = DUTY_WEAK; break;    // LEVEL3: 最弱
    default: duty = DUTY_DEFAULT; break;
  }
  currentLevel = level;
...
void parseLevelCommand(String value) {
  if (value.startsWith("LEVEL:")) {
    ...
    if (level >= 0 && level <= 3 && level != lastLevel) {
      lastLevel = level;
      setVibration(level);
      digitalWrite(LED_PIN, (level != 2) ? HIGH : LOW);
    }
  }
}
...
void setup() {
  ...
  Serial.println("\\n========================================");
  Serial.println(" 跳蛋振动控制器 v1.5");
  Serial.println(" LEVEL:0→最强 1→中等 2→中等 3→最弱");
  Serial.println("========================================\\n");
  ...
  ledcWrite(MOTOR_PIN, 0);  // 初始停止
  
  // 开机测试
  Serial.println("[自检] 马达振动测试...");
  ledcWrite(MOTOR_PIN, DUTY_STRONG);
  delay(1000);
  ledcWrite(MOTOR_PIN, DUTY_MED);
  delay(1000);
  ledcWrite(MOTOR_PIN, DUTY_WEAK);
  delay(1000);
  ledcWrite(MOTOR_PIN, 0);  // 停止
  delay(500);
  Serial.println("[自检] 完成");
  
  // 设置默认振动（无传感器数据时也振动）
  Serial.println("[默认] 设置默认振动等级...");
  setVibration(3);  // 默认LEVEL3（最弱振动）
